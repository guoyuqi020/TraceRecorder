...
static void addTraceRecorderPass(const PassManagerBuilder &Builder,
                                   legacy::PassManagerBase &PM) {
  PM.add(createTraceRecorderLegacyPassPass());
}
...
#include "llvm/Support/Extension.def"
  ...
  if (!CodeGenOpts.DisableLLVMPasses) {
    ...
    if (LangOpts.Sanitize.has(SanitizerKind::Trace)) {
      PB.registerOptimizerLastEPCallback(
          [](ModulePassManager &MPM, PassBuilder::OptimizationLevel Level) {
            MPM.addPass(TraceRecorderPass());
            MPM.addPass(
                createModuleToFunctionPassAdaptor(TraceRecorderPass()));
          });
    }
    if (LangOpts.Sanitize.has(SanitizerKind::Thread)) {
      PB.registerOptimizerLastEPCallback(
