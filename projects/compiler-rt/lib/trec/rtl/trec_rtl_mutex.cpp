...
void MutexDestroy(ThreadState *thr, uptr pc, uptr addr, u32 flagz) {
  ...
  if (flags()->report_destroy_locked && s->owner_tid != SyncVar::kInvalidTid &&
...
void MutexPostLock(ThreadState *thr, uptr pc, uptr addr, u32 flagz, int rec) {
  ...
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::MutexLock,
    ...
    __trec_metadata::MutexMeta meta(addr,pc);
...
int MutexUnlock(ThreadState *thr, uptr pc, uptr addr, u32 flagz) {
  ...
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::MutexUnlock,
        atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
        thr->tctx->metadata_offset);
...
void AcquireGlobal(ThreadState *thr, uptr pc) {
  ...
  if (thr->ignore_sync)
    return;
  ThreadRegistryLock l(ctx->thread_registry);
  ctx->thread_registry->RunCallbackForEachThreadLocked(UpdateClockCallback,
                                                       thr);
}
