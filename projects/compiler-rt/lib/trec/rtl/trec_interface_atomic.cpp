...
static T AtomicLoad(ThreadState *thr, uptr pc, const volatile T *a, morder mo,
  ...
  if (!IsAcquireOrder(mo)) {
    ...
    if (LIKELY(SizeLog<T>() < 4)) {
      MemoryReadAtomic(thr, pc, (uptr)a + 8, SizeLog<T>(), isPtr, (uptr)v);
    } else {
      MemoryReadAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)v);
      MemoryReadAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)(v >> 64));
    }
    return v;
  }
...
static void AtomicStore(ThreadState *thr, uptr pc, volatile T *a, T v,
  ...
  RecordFuncEntry(thr, pc);
  if (LIKELY(SizeLog<T>() < 4)) {
    MemoryWriteAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)v, {0, 1},
                      {0, 2});
  } else {
    MemoryWriteAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)v, {0, 1}, {0, 2});
    MemoryWriteAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)(v >> 64), {0, 1},
                      {0, 2});
  }
  RecordFuncExit(thr);
  // This fast-path is critical for performance.
...
static T AtomicRMW(ThreadState *thr, uptr pc, volatile T *a, T v, morder mo,
  ...
  v = F(a, v);
  if (LIKELY(SizeLog<T>() < 4)) {
    MemoryReadAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)v, {0, 1});
    MemoryWriteAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)(*a), {0, 1},
                      {0, 2});
  } else {
    MemoryReadAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)v, {0, 1});
    MemoryReadAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)(v >> 64), {0, 1});
    MemoryWriteAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)(*a), {0, 1}, {0, 2});
    MemoryWriteAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)((*a) >> 64),
                      {0, 1}, {0, 2});
  }
  if (s)
    s->mtx.Unlock();
...
static bool AtomicCAS(ThreadState *thr, uptr pc, volatile T *a, T *c, T v,
  ...
  T cc = *c;
  if (LIKELY(SizeLog<T>() < 4)) {
    MemoryReadAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)cc);
  } else {
    MemoryReadAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)cc);
    MemoryReadAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)(cc >> 64));
  }
  ...
  if (LIKELY(SizeLog<T>() < 4)) {
    MemoryReadAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)pr);
  } else {
    MemoryReadAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)pr);
    MemoryReadAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)(pr >> 64));
  }
  CondBranch(thr, pc);
  if (pr == cc) {
    if (LIKELY(SizeLog<T>() < 4)) {
      MemoryWriteAtomic(thr, pc, (uptr)a, SizeLog<T>(), isPtr, (uptr)v);
    } else {
      MemoryWriteAtomic(thr, pc, (uptr)a, 3, isPtr, (uptr)v);
      MemoryWriteAtomic(thr, pc, (uptr)a + 8, 3, isPtr, (uptr)(v >> 64));
    }
    return true;
  }
  *c = pr;
...
#define SCOPED_ATOMIC_TREC(func, isPtr, ...)                    \
  ...
  mo = convert_morder(mo);                                      \
  AtomicStatInc(thr, sizeof(*a), mo, StatAtomic##func);         \
  ScopedAtomic sa(thr, callpc - 1, a, mo, __func__);            \
  return Atomic##func(thr, callpc - 1, __VA_ARGS__, isPtr);     \
  /**/
