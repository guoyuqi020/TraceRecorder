...
#include "trec_defs.h"
#include "trec_clock.h"
#include "trec_mutex.h"
#include "trec_dense_alloc.h"

namespace __trec {

// These need to match __trec_mutex_* flags defined in trec_interface.h.
// See documentation there as well.
enum MutexFlags {
  MutexFlagLinkerInit          = 1 << 0, // __trec_mutex_linker_init
  MutexFlagWriteReentrant      = 1 << 1, // __trec_mutex_write_reentrant
  MutexFlagReadReentrant       = 1 << 2, // __trec_mutex_read_reentrant
  MutexFlagReadLock            = 1 << 3, // __trec_mutex_read_lock
  MutexFlagTryLock             = 1 << 4, // __trec_mutex_try_lock
  MutexFlagTryLockFailed       = 1 << 5, // __trec_mutex_try_lock_failed
  MutexFlagRecursiveLock       = 1 << 6, // __trec_mutex_recursive_lock
  MutexFlagRecursiveUnlock     = 1 << 7, // __trec_mutex_recursive_unlock
  MutexFlagNotStatic           = 1 << 8, // __trec_mutex_not_static
...
/* MetaMap allows to map arbitrary user pointers onto various descriptors.
   ...
   It uses 1/2 direct shadow, see trec_platform.h.
*/
class MetaMap {
 public:
  MetaMap();
