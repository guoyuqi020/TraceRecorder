...
static void init(ThreadState *thr, uptr pc, int fd, FdSync *s,
  ...
  if (write) {
    ...
    OnUserAlloc(thr, pc, (uptr)d, (1 << kSizeLog8), true);
    MemoryWrite(thr, pc, (uptr)d, kSizeLog8, false, (uptr)(*((uptr *)d)));
  }
}
...
void FdAcquire(ThreadState *thr, uptr pc, int fd) {
  ...
  MemoryRead(thr, pc, (uptr)d, kSizeLog8, false, (uptr)(*((uptr *)d)));
  if (s)
...
void FdDup(ThreadState *thr, uptr pc, int oldfd, int newfd, bool write) {
  ...
  MemoryRead(thr, pc, (uptr)od, kSizeLog8, false, (uptr)(*((uptr *)od)));
