...
void __trec_java_init(jptr heap_begin, jptr heap_size) {
  SCOPED_JAVA_FUNC(__trec_java_init);
...
int  __trec_java_fini() {
  SCOPED_JAVA_FUNC(__trec_java_fini);
  DPrintf("#%d: java_fini()\n", thr->tid);
  CHECK_NE(jctx, 0);
...
void __trec_java_alloc(jptr ptr, jptr size) {
  SCOPED_JAVA_FUNC(__trec_java_alloc);
...
void __trec_java_free(jptr ptr, jptr size) {
  SCOPED_JAVA_FUNC(__trec_java_free);
...
void __trec_java_move(jptr src, jptr dst, jptr size) {
  SCOPED_JAVA_FUNC(__trec_java_move);
...
jptr __trec_java_find(jptr *from_ptr, jptr to) {
  SCOPED_JAVA_FUNC(__trec_java_find);
...
void __trec_java_finalize() {
  SCOPED_JAVA_FUNC(__trec_java_finalize);
  DPrintf("#%d: java_mutex_finalize()\n", thr->tid);
  AcquireGlobal(thr, 0);
}

void __trec_java_mutex_lock(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_mutex_lock);
...
void __trec_java_mutex_unlock(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_mutex_unlock);
...
void __trec_java_mutex_read_lock(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_mutex_read_lock);
...
void __trec_java_mutex_read_unlock(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_mutex_read_unlock);
...
void __trec_java_mutex_lock_rec(jptr addr, int rec) {
  SCOPED_JAVA_FUNC(__trec_java_mutex_lock_rec);
...
int __trec_java_mutex_unlock_rec(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_mutex_unlock_rec);
...
void __trec_java_acquire(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_acquire);
...
void __trec_java_release(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_release);
...
void __trec_java_release_store(jptr addr) {
  SCOPED_JAVA_FUNC(__trec_java_release);
