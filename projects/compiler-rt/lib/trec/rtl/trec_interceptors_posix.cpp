...
static void LongJmp(ThreadState *thr, uptr *env) {
  ...
  Printf("TraceRecorder: can't find longjmp buf\n");
  CHECK(0);
}

// FIXME: put everything below into a common extern "C" block?
extern "C" void __trec_setjmp(uptr sp) {
  cur_thread_init();
...
#define TREC_INTERCEPTOR_SETJMP_(x) __interceptor_##x
#define TREC_INTERCEPTOR_SETJMP__(x) TREC_INTERCEPTOR_SETJMP_(x)
#define TREC_INTERCEPTOR_SETJMP TREC_INTERCEPTOR_SETJMP__(setjmp_symname)
#define TREC_INTERCEPTOR_SIGSETJMP TREC_INTERCEPTOR_SETJMP__(sigsetjmp_symname)

#define TREC_STRING_SETJMP SANITIZER_STRINGIFY(setjmp_symname)
#define TREC_STRING_SIGSETJMP SANITIZER_STRINGIFY(sigsetjmp_symname)

// Not called.  Merely to satisfy TREC_INTERCEPT().
extern "C" SANITIZER_INTERFACE_ATTRIBUTE int TREC_INTERCEPTOR_SETJMP(void *env);
extern "C" int TREC_INTERCEPTOR_SETJMP(void *env) {
  CHECK(0);
  return 0;
}

// FIXME: any reason to have a separate declaration?
extern "C" SANITIZER_INTERFACE_ATTRIBUTE int __interceptor__setjmp(void *env);
extern "C" int __interceptor__setjmp(void *env) {
  CHECK(0);
  return 0;
}

extern "C" SANITIZER_INTERFACE_ATTRIBUTE int TREC_INTERCEPTOR_SIGSETJMP(
    void *env);
extern "C" int TREC_INTERCEPTOR_SIGSETJMP(void *env) {
  CHECK(0);
  return 0;
}

#if !SANITIZER_NETBSD
extern "C" SANITIZER_INTERFACE_ATTRIBUTE int __interceptor___sigsetjmp(
    void *env);
...
TREC_INTERCEPTOR(int, munmap, void *addr, long_t sz) {
  SCOPED_TREC_INTERCEPTOR(munmap, addr, sz);
...
TREC_INTERCEPTOR(int, pthread_create, void *th, void *attr,
  ...
  int res = -1;
  {
    ...
    ScopedIgnoreInterceptors ignore;
    ThreadIgnoreBegin(thr, caller_pc);
    res = REAL(pthread_create)(th, attr, __trec_thread_start_func, &p);
    ThreadIgnoreEnd(thr, caller_pc);
  }
  if (res == 0) {
    int tid =
...
INTERCEPTOR(int, pthread_cond_init, void *c, void *a) {
  ...
  MemoryAccessRange(thr, caller_pc, (uptr)c, sizeof(uptr), true);
...
INTERCEPTOR(int, pthread_cond_signal, void *c) {
  ...
  MemoryAccessRange(thr, caller_pc, (uptr)c, sizeof(uptr), false);
...
TREC_INTERCEPTOR(int, pthread_mutex_init, void *m, void *a) {
  ...
  if (res == 0) {
    ...
    MutexCreate(thr, caller_pc, (uptr)m, flagz);
  }
  return res;
}
...
TREC_INTERCEPTOR(int, pthread_mutex_trylock, void *m) {
  ...
  if (res == 0 || res == errno_EOWNERDEAD)
    MutexPostLock(thr, caller_pc, (uptr)m, MutexFlagTryLock);
  return res;
}
...
TREC_INTERCEPTOR(int, pthread_spin_lock, void *m) {
  ...
  if (res == 0) {
    MutexPostLock(thr, caller_pc, (uptr)m);
  }
  return res;
}
...
TREC_INTERCEPTOR(int, pthread_spin_unlock, void *m) {
  ...
  MutexUnlock(thr, caller_pc, (uptr)m);
...
TREC_INTERCEPTOR(int, pthread_rwlock_rdlock, void *m) {
  ...
  if (res == 0) {
    MutexPostReadLock(thr, caller_pc, (uptr)m);
  }
  return res;
}
...
TREC_INTERCEPTOR(int, pthread_rwlock_tryrdlock, void *m) {
  ...
  if (res == 0) {
    MutexPostReadLock(thr, caller_pc, (uptr)m, MutexFlagTryLock);
  }
  return res;
}
...
TREC_INTERCEPTOR(int, pthread_rwlock_unlock, void *m) {
  ...
  MutexReadOrWriteUnlock(thr, caller_pc, (uptr)m);
...
TREC_INTERCEPTOR(int, pthread_barrier_init, void *b, void *a, unsigned count) {
  ...
  // MemoryWrite(thr, caller_pc, (uptr)b, kSizeLog1);
...
TREC_INTERCEPTOR(int, pthread_barrier_wait, void *b) {
  ...
  Release(thr, caller_pc, (uptr)b);
  // MemoryRead(thr, caller_pc, (uptr)b, kSizeLog1);
  int res = REAL(pthread_barrier_wait)(b);
  // MemoryRead(thr, caller_pc, (uptr)b, kSizeLog1);
  if (res == 0 || res == PTHREAD_BARRIER_SERIAL_THREAD) {
    Acquire(thr, caller_pc, (uptr)b);
  }
  return res;
}
#endif
...
TREC_INTERCEPTOR(int, pthread_once, void *o, void (*f)()) {
  ...
  if (v == 0 &&
    ...
    if (!thr->in_ignored_lib)
      Acquire(thr, caller_pc, (uptr)o);
  }
  return 0;
}

#if SANITIZER_LINUX && !SANITIZER_ANDROID
TREC_INTERCEPTOR(int, __fxstat, int version, int fd, void *buf) {
  SCOPED_TREC_INTERCEPTOR(__fxstat, version, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(__fxstat)(version, fd, buf);
}
#define TREC_MAYBE_INTERCEPT___FXSTAT TREC_INTERCEPT(__fxstat)
#else
#define TREC_MAYBE_INTERCEPT___FXSTAT
#endif

TREC_INTERCEPTOR(int, fstat, int fd, void *buf) {
#if SANITIZER_FREEBSD || SANITIZER_MAC || SANITIZER_ANDROID || SANITIZER_NETBSD
  SCOPED_TREC_INTERCEPTOR(fstat, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(fstat)(fd, buf);
#else
  SCOPED_TREC_INTERCEPTOR(__fxstat, 0, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
...
#if SANITIZER_LINUX && !SANITIZER_ANDROID
TREC_INTERCEPTOR(int, __fxstat64, int version, int fd, void *buf) {
  SCOPED_TREC_INTERCEPTOR(__fxstat64, version, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(__fxstat64)(version, fd, buf);
}
#define TREC_MAYBE_INTERCEPT___FXSTAT64 TREC_INTERCEPT(__fxstat64)
#else
#define TREC_MAYBE_INTERCEPT___FXSTAT64
#endif

#if SANITIZER_LINUX && !SANITIZER_ANDROID
TREC_INTERCEPTOR(int, fstat64, int fd, void *buf) {
  SCOPED_TREC_INTERCEPTOR(__fxstat64, 0, fd, buf);
  if (fd > 0)
    FdAccess(thr, pc, fd);
  return REAL(__fxstat64)(0, fd, buf);
}
#define TREC_MAYBE_INTERCEPT_FSTAT64 TREC_INTERCEPT(fstat64)
#else
#define TREC_MAYBE_INTERCEPT_FSTAT64
#endif

TREC_INTERCEPTOR(int, open, const char *name, int oflag, ...) {
  va_list ap;
  va_start(ap, oflag);
  mode_t mode = va_arg(ap, int);
  va_end(ap);
  SCOPED_TREC_INTERCEPTOR(open, name, oflag, mode);
...
#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, open64, const char *name, int oflag, ...) {
  va_list ap;
  va_start(ap, oflag);
  mode_t mode = va_arg(ap, int);
  va_end(ap);
  SCOPED_TREC_INTERCEPTOR(open64, name, oflag, mode);
  READ_STRING(thr, pc, name, 0);
...
#define TREC_MAYBE_INTERCEPT_OPEN64 TREC_INTERCEPT(open64)
#else
#define TREC_MAYBE_INTERCEPT_OPEN64
#endif

TREC_INTERCEPTOR(int, creat, const char *name, int mode) {
  SCOPED_TREC_INTERCEPTOR(creat, name, mode);
  READ_STRING(thr, pc, name, 0);
...
#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, creat64, const char *name, int mode) {
  SCOPED_TREC_INTERCEPTOR(creat64, name, mode);
  READ_STRING(thr, pc, name, 0);
...
#define TREC_MAYBE_INTERCEPT_CREAT64 TREC_INTERCEPT(creat64)
#else
#define TREC_MAYBE_INTERCEPT_CREAT64
#endif

TREC_INTERCEPTOR(int, dup, int oldfd) {
  SCOPED_TREC_INTERCEPTOR(dup, oldfd);
...
TREC_INTERCEPTOR(int, dup2, int oldfd, int newfd) {
  SCOPED_TREC_INTERCEPTOR(dup2, oldfd, newfd);
...
#if !SANITIZER_MAC
TREC_INTERCEPTOR(int, dup3, int oldfd, int newfd, int flags) {
  SCOPED_TREC_INTERCEPTOR(dup3, oldfd, newfd, flags);
...
#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, eventfd, unsigned initval, int flags) {
  SCOPED_TREC_INTERCEPTOR(eventfd, initval, flags);
  int fd = REAL(eventfd)(initval, flags);
  if (fd >= 0)
    FdEventCreate(thr, pc, fd);
  return fd;
}
#define TREC_MAYBE_INTERCEPT_EVENTFD TREC_INTERCEPT(eventfd)
#else
#define TREC_MAYBE_INTERCEPT_EVENTFD
#endif

#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, signalfd, int fd, void *mask, int flags) {
  SCOPED_TREC_INTERCEPTOR(signalfd, fd, mask, flags);
  if (fd >= 0)
    FdClose(thr, pc, fd);
...
#define TREC_MAYBE_INTERCEPT_SIGNALFD TREC_INTERCEPT(signalfd)
#else
#define TREC_MAYBE_INTERCEPT_SIGNALFD
#endif

#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, inotify_init, int fake) {
  SCOPED_TREC_INTERCEPTOR(inotify_init, fake);
  int fd = REAL(inotify_init)(fake);
  if (fd >= 0)
    FdInotifyCreate(thr, pc, fd);
  return fd;
}
#define TREC_MAYBE_INTERCEPT_INOTIFY_INIT TREC_INTERCEPT(inotify_init)
#else
#define TREC_MAYBE_INTERCEPT_INOTIFY_INIT
#endif

#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, inotify_init1, int flags) {
  SCOPED_TREC_INTERCEPTOR(inotify_init1, flags);
  int fd = REAL(inotify_init1)(flags);
  if (fd >= 0)
    FdInotifyCreate(thr, pc, fd);
  return fd;
}
#define TREC_MAYBE_INTERCEPT_INOTIFY_INIT1 TREC_INTERCEPT(inotify_init1)
#else
#define TREC_MAYBE_INTERCEPT_INOTIFY_INIT1
#endif

TREC_INTERCEPTOR(int, socket, int domain, int type, int protocol) {
  SCOPED_TREC_INTERCEPTOR(socket, domain, type, protocol);
...
TREC_INTERCEPTOR(int, socketpair, int domain, int type, int protocol, int *fd) {
  SCOPED_TREC_INTERCEPTOR(socketpair, domain, type, protocol, fd);
...
TREC_INTERCEPTOR(int, connect, int fd, void *addr, unsigned addrlen) {
  SCOPED_TREC_INTERCEPTOR(connect, fd, addr, addrlen);
...
TREC_INTERCEPTOR(int, bind, int fd, void *addr, unsigned addrlen) {
  SCOPED_TREC_INTERCEPTOR(bind, fd, addr, addrlen);
...
TREC_INTERCEPTOR(int, listen, int fd, int backlog) {
  SCOPED_TREC_INTERCEPTOR(listen, fd, backlog);
...
TREC_INTERCEPTOR(int, close, int fd) {
  SCOPED_TREC_INTERCEPTOR(close, fd);
  if (fd >= 0)
    FdClose(thr, pc, fd);
...
#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, __close, int fd) {
  SCOPED_TREC_INTERCEPTOR(__close, fd);
  if (fd >= 0)
    FdClose(thr, pc, fd);
  return REAL(__close)(fd);
}
#define TREC_MAYBE_INTERCEPT___CLOSE TREC_INTERCEPT(__close)
#else
#define TREC_MAYBE_INTERCEPT___CLOSE
#endif

// glibc guts
#if SANITIZER_LINUX && !SANITIZER_ANDROID
TREC_INTERCEPTOR(void, __res_iclose, void *state, bool free_addr) {
  SCOPED_TREC_INTERCEPTOR(__res_iclose, state, free_addr);
  int fds[64];
...
#define TREC_MAYBE_INTERCEPT___RES_ICLOSE TREC_INTERCEPT(__res_iclose)
#else
#define TREC_MAYBE_INTERCEPT___RES_ICLOSE
#endif

TREC_INTERCEPTOR(int, pipe, int *pipefd) {
  SCOPED_TREC_INTERCEPTOR(pipe, pipefd);
...
#if !SANITIZER_MAC
TREC_INTERCEPTOR(int, pipe2, int *pipefd, int flags) {
  SCOPED_TREC_INTERCEPTOR(pipe2, pipefd, flags);
...
TREC_INTERCEPTOR(int, unlink, char *path) {
  SCOPED_TREC_INTERCEPTOR(unlink, path);
  Release(thr, pc, File2addr(path));
  int res = REAL(unlink)(path);
  return res;
}

TREC_INTERCEPTOR(void *, tmpfile, int fake) {
  SCOPED_TREC_INTERCEPTOR(tmpfile, fake);
...
#if SANITIZER_LINUX
TREC_INTERCEPTOR(void *, tmpfile64, int fake) {
  SCOPED_TREC_INTERCEPTOR(tmpfile64, fake);
...
#define TREC_MAYBE_INTERCEPT_TMPFILE64 TREC_INTERCEPT(tmpfile64)
#else
#define TREC_MAYBE_INTERCEPT_TMPFILE64
#endif
...
TREC_INTERCEPTOR(int, rmdir, char *path) {
  SCOPED_TREC_INTERCEPTOR(rmdir, path);
  Release(thr, pc, Dir2addr(path));
  int res = REAL(rmdir)(path);
  return res;
}

TREC_INTERCEPTOR(int, closedir, void *dirp) {
  SCOPED_TREC_INTERCEPTOR(closedir, dirp);
  if (dirp) {
    int fd = dirfd(dirp);
    FdClose(thr, pc, fd);
  }
...
#if SANITIZER_LINUX
TREC_INTERCEPTOR(int, epoll_create, int size) {
  SCOPED_TREC_INTERCEPTOR(epoll_create, size);
...
TREC_INTERCEPTOR(int, epoll_create1, int flags) {
  SCOPED_TREC_INTERCEPTOR(epoll_create1, flags);
...
TREC_INTERCEPTOR(int, epoll_ctl, int epfd, int op, int fd, void *ev) {
  SCOPED_TREC_INTERCEPTOR(epoll_ctl, epfd, op, fd, ev);
  if (epfd >= 0)
...
TREC_INTERCEPTOR(int, epoll_wait, int epfd, void *ev, int cnt, int timeout) {
  SCOPED_TREC_INTERCEPTOR(epoll_wait, epfd, ev, cnt, timeout);
  if (epfd >= 0)
    FdAccess(thr, pc, epfd);
...
TREC_INTERCEPTOR(int, epoll_pwait, int epfd, void *ev, int cnt, int timeout,
                 void *sigmask) {
  SCOPED_TREC_INTERCEPTOR(epoll_pwait, epfd, ev, cnt, timeout, sigmask);
  if (epfd >= 0)
    FdAccess(thr, pc, epfd);
...
#define TREC_MAYBE_INTERCEPT_EPOLL \
  TREC_INTERCEPT(epoll_create);    \
  TREC_INTERCEPT(epoll_create1);   \
  TREC_INTERCEPT(epoll_ctl);       \
  TREC_INTERCEPT(epoll_wait);      \
  TREC_INTERCEPT(epoll_pwait)
#else
#define TREC_MAYBE_INTERCEPT_EPOLL
#endif
...
static void CallUserSignalHandler(ThreadState *thr, bool sync, bool acquire,
  ...
  // trec reports false positive in such case.
...
TREC_INTERCEPTOR(int, fork, int fake) {
  ...
  internal_snprintf(temp_dir_path, 255, "%s/temp_%d_%d", ctx->trace_dir,
                    internal_getpid(), cur_forked_cnt);
  ...
  ctx->CopyDir(temp_dir_path, thr->tid);
  {
  ...
  if (pid == 0) {
    ...
    FdOnFork(thr, pc);
    ctx->InheritDir(temp_dir_path, internal_getpid());
  } else if (pid > 0) {
    // parent
...
TREC_INTERCEPTOR(int, dl_iterate_phdr, dl_iterate_phdr_cb_t cb, void *data) {
  SCOPED_TREC_INTERCEPTOR(dl_iterate_phdr, cb, data);
...
#define COMMON_INTERCEPTOR_LIBRARY_UNLOADED() libignore()->OnLibraryUnloaded()

#define COMMON_INTERCEPTOR_ACQUIRE(ctx, u) \
  Acquire(((TrecInterceptorContext *)ctx)->thr, pc, u)

#define COMMON_INTERCEPTOR_RELEASE(ctx, u) \
  Release(((TrecInterceptorContext *)ctx)->thr, pc, u)
...
#define COMMON_INTERCEPTOR_USER_CALLBACK_START() \
  SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_START()

#define COMMON_INTERCEPTOR_USER_CALLBACK_END() \
  SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_END()
...
//    is empty for trec (meant only for msan).
...
TREC_INTERCEPTOR(void *, __tls_get_addr, void *arg) {
...
void InitializeInterceptors() {
...
#if !SANITIZER_MAC
  // We can not use TREC_INTERCEPT to get setjmp addr,
  // because it does &setjmp and setjmp is not present in some versions of libc.
  using __interception::InterceptFunction;
  InterceptFunction(TREC_STRING_SETJMP, (uptr *)&REAL(setjmp_symname), 0, 0);
  InterceptFunction("_setjmp", (uptr *)&REAL(_setjmp), 0, 0);
  InterceptFunction(TREC_STRING_SIGSETJMP, (uptr *)&REAL(sigsetjmp_symname), 0,
                    0);
#if !SANITIZER_NETBSD
  ...
  TREC_INTERCEPT_VER(pthread_cond_init, PTHREAD_ABI_BASE);
  TREC_INTERCEPT_VER(pthread_cond_signal, PTHREAD_ABI_BASE);
  TREC_INTERCEPT_VER(pthread_cond_broadcast, PTHREAD_ABI_BASE);
  TREC_INTERCEPT_VER(pthread_cond_wait, PTHREAD_ABI_BASE);
  TREC_INTERCEPT_VER(pthread_cond_timedwait, PTHREAD_ABI_BASE);
  TREC_INTERCEPT_VER(pthread_cond_destroy, PTHREAD_ABI_BASE);
  ...
  TREC_INTERCEPT(fstat);
  TREC_MAYBE_INTERCEPT___FXSTAT;
  TREC_MAYBE_INTERCEPT_FSTAT64;
  TREC_MAYBE_INTERCEPT___FXSTAT64;
  TREC_INTERCEPT(open);
  TREC_MAYBE_INTERCEPT_OPEN64;
  TREC_INTERCEPT(creat);
  TREC_MAYBE_INTERCEPT_CREAT64;
  TREC_INTERCEPT(dup);
  TREC_INTERCEPT(dup2);
  TREC_INTERCEPT(dup3);
  TREC_MAYBE_INTERCEPT_EVENTFD;
  TREC_MAYBE_INTERCEPT_SIGNALFD;
  TREC_MAYBE_INTERCEPT_INOTIFY_INIT;
  TREC_MAYBE_INTERCEPT_INOTIFY_INIT1;
  TREC_INTERCEPT(socket);
  TREC_INTERCEPT(socketpair);
  TREC_INTERCEPT(connect);
  TREC_INTERCEPT(bind);
  TREC_INTERCEPT(listen);
  TREC_MAYBE_INTERCEPT_EPOLL;
  TREC_INTERCEPT(close);
  TREC_MAYBE_INTERCEPT___CLOSE;
  TREC_MAYBE_INTERCEPT___RES_ICLOSE;
  TREC_INTERCEPT(pipe);
  TREC_INTERCEPT(pipe2);

  TREC_INTERCEPT(unlink);
  TREC_INTERCEPT(tmpfile);
  TREC_MAYBE_INTERCEPT_TMPFILE64;
  TREC_INTERCEPT(abort);
  TREC_INTERCEPT(rmdir);
  TREC_INTERCEPT(closedir);
...
#if !SANITIZER_ANDROID
  TREC_INTERCEPT(dl_iterate_phdr);
#endif
...
//    output tests.
// 3. Then we switched to atomics+sched_yield. But this produced tons of trec-
//    visible events, which lead to "failed to restore stack trace" failures.
// Note that no_sanitize_thread attribute does not turn off atomic interception
// so attaching it to the function defined in user code does not help.
// That's why we now have what we have.
extern "C" SANITIZER_INTERFACE_ATTRIBUTE void __trec_testonly_barrier_init(
    u64 *barrier, u32 count) {
  if (count >= (1 << 8)) {
...
extern "C" SANITIZER_INTERFACE_ATTRIBUTE void __trec_testonly_barrier_wait(
    u64 *barrier) {
