...
void __trec_on_report(const ReportDesc *rep) { (void)rep; }
...
static void StackStripMain(SymbolizedStack *frames) {
  ...
  if (last2 && 0 == internal_strcmp(last2, "main")) {
    ...
    // Strip our internal thread start routine.
             ...
             0 == internal_strcmp(last, "__trec_thread_start_func")) {
    last_frame->ClearAll();
    last_frame2->next = nullptr;
...
static bool FindRacyStacks(const RacyStacks &hash) {
  for (uptr i = 0; i < ctx->racy_stacks.Size(); i++) {
    if (hash == ctx->racy_stacks[i]) {
      VPrintf(2, "TraceRecorder: suppressing report as doubled (stack)\n");
      return true;
    }
  }
  return false;
...
static bool FindRacyAddress(const RacyAddress &ra0) {
  for (uptr i = 0; i < ctx->racy_addresses.Size(); i++) {
    ...
    if (maxbeg < minend) {
      VPrintf(2, "TraceRecorder: suppressing report as doubled (addr)\n");
      return true;
    }
  }
  return false;
...
void ReportRace(ThreadState *thr) {
  ...
  if (toppc >> kEventPCBits) {
    ...
    // a __trec_symbolize_external callback and a race during a range
    // memory access larger than 8 bytes. MemoryAccessRange adds the current PC
    // to the trace and starts processing memory accesses. A first memory access
    ...
    // __trec_symbolize_external_ex. __trec_symbolize_external_ex is
    // within its rights to crash since the PC is completely bogus.
    // test/trec/double_race.cpp contains a test case for this.
    toppc = 0;
  }
