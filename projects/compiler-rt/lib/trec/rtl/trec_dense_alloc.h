...
//===----------------------------------------------------------------------===//
#ifndef TREC_DENSE_ALLOC_H
#define TREC_DENSE_ALLOC_H
...
class DenseSlabAlloc {
 ...
 private:
  ...
  void Refill(Cache *c) {
    ...
    if (freelist_ == 0) {
      if (fillpos_ == kL1Size) {
        Printf("TraceRecorder: %s overflow (%zu*%zu). Dying.\n",
            name_, kL1Size, kL2Size);
        Die();
      }
      VPrintf(2, "TraceRecorder: growing %s: %zu out of %zu*%zu\n",
