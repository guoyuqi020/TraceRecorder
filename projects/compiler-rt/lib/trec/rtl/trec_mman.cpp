...
void user_free(ThreadState *thr, uptr pc, void *p, bool signal,
  ...
  if (ctx && ctx->initialized) {
    OnUserFree(thr, pc, (uptr)p, true, record_trace);
  }
...
void *user_alloc(ThreadState *thr, uptr pc, uptr sz) {
  return SetErrnoOnNull(
      user_alloc_internal(thr, pc, sz, kDefaultAlignment, true, true));
}
...
void *user_calloc(ThreadState *thr, uptr pc, uptr size, uptr n) {
  ...
  void *p =
      user_alloc_internal(thr, pc, n * size, kDefaultAlignment, true, true);
  if (p)
...
void OnUserAlloc(ThreadState *thr, uptr pc, uptr p, uptr sz, bool write,
  ...
  if (record_trace && thr && thr->tctx && LIKELY(ctx->flags.output_trace) &&
      (IsHeapMem(p) || (!ctx->flags.no_stack && IsStackMem(p))) &&
      !IsInternalMem(p, sz)) {
    ...
    __trec_metadata::MemAllocMeta meta(sz, p, pc);
...
void OnUserFree(ThreadState *thr, uptr pc, uptr p, bool write,
  ...
  if (record_trace && thr && thr->tctx && LIKELY(ctx->flags.output_trace) &&
    ...
    thr->tctx->put_trace(&e, sizeof(__trec_trace::Event));
    __trec_metadata::MemFreeMeta meta(sz, p, 0, 0, pc);
    thr->tctx->put_metadata(&meta, sizeof(meta));
...
void *user_realloc(ThreadState *thr, uptr pc, void *p, uptr sz) {
  ...
  void *new_p = user_alloc_internal(thr, pc, sz, kDefaultAlignment, true, true);
  if (new_p) {
...
void *user_memalign(ThreadState *thr, uptr pc, uptr align, uptr sz) {
  ...
  return SetErrnoOnNull(user_alloc_internal(thr, pc, sz, align, true, true));
}
...
int user_posix_memalign(ThreadState *thr, uptr pc, void **memptr, uptr align,
  ...
  void *ptr = user_alloc_internal(thr, pc, sz, align, true, true);
  if (UNLIKELY(!ptr))
...
void *user_valloc(ThreadState *thr, uptr pc, uptr sz) {
  return SetErrnoOnNull(
      user_alloc_internal(thr, pc, sz, GetPageSizeCached(), true, true));
}
...
void *user_pvalloc(ThreadState *thr, uptr pc, uptr sz) {
  ...
  return SetErrnoOnNull(user_alloc_internal(thr, pc, sz, PageSize, true, true));
}
