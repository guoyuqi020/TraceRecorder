...
TREC_INTERCEPTOR(kern_return_t, mach_vm_allocate, vm_map_t target,
                 mach_vm_address_t *address, mach_vm_size_t size, int flags) {
  SCOPED_TREC_INTERCEPTOR(mach_vm_allocate, target, address, size, flags);
...
TREC_INTERCEPTOR(kern_return_t, mach_vm_deallocate, vm_map_t target,
                 mach_vm_address_t address, mach_vm_size_t size) {
  SCOPED_TREC_INTERCEPTOR(mach_vm_deallocate, target, address, size);
  if (target != mach_task_self())
