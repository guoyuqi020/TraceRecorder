...
void CondBranch(ThreadState *thr, uptr pc);
void FuncParam(ThreadState *thr, char param_idx, uptr src_addr, char src_idx);
void FuncExitParam(ThreadState *thr, uptr src_addr, char src_idx);
void FuncEnterOrder(ThreadState *thr, char idx);
...
void MemoryAccessRange(ThreadState *thr, uptr pc, uptr addr, uptr size,
                       bool is_write);
void MemoryAccessRangeStep(ThreadState *thr, uptr pc, uptr addr, uptr size,
                           uptr step, bool is_write);
...
void ALWAYS_INLINE MemoryRead(ThreadState *thr, uptr pc, uptr addr,
                              int kAccessSizeLog, bool isPtr = false,
                              uptr val = 0,
...
void ALWAYS_INLINE
MemoryWrite(ThreadState *thr, uptr pc, uptr addr, int kAccessSizeLog,
            bool isPtr = false, uptr val = 0,
            __trec_metadata::SourceAddressInfo SAI_addr = {0, 0},
            __trec_metadata::SourceAddressInfo SAI_val = {0, 0}) {
...
void RecordFuncEntry(ThreadState *thr, uptr pc);
...
extern "C" void __trec_trace_switch();
...
#if !SANITIZER_GO
    HACKY_CALL(__trec_trace_switch);
#else
    TraceSwitch(thr);
#endif
  }
