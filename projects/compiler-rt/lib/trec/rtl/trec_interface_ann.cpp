...
void PrintMatchedBenignRaces() {
  ...
  CollectMatchedBenignRaces(&add_matched, &unique_count, &add_count,
      &ExpectRace::addcount);
  if (hit_matched.Size()) {
    Printf("TraceRecorder: Matched %d \"benign\" races (pid=%d):\n",
  ...
  if (hit_matched.Size()) {
    Printf("TraceRecorder: Annotated %d \"benign\" races, %d unique"
           " (pid=%d):\n",
...
static void ReportMissedExpectedRace(ExpectRace *race) {
  Printf("==================\n");
  Printf("WARNING: TraceRecorder: missed expected data race\n");
...
// atomic operations, which should be handled by TraceRecorder correctly.
...
const char INTERFACE_ATTRIBUTE* TraceRecorderQuery(const char *query) {
...
INTERFACE_ATTRIBUTE
void __trec_mutex_create(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_create);
  MutexCreate(thr, pc, (uptr)m, flagz & MutexCreationFlagMask);
}

INTERFACE_ATTRIBUTE
void __trec_mutex_destroy(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_destroy);
  MutexDestroy(thr, pc, (uptr)m, flagz);
}

INTERFACE_ATTRIBUTE
void __trec_mutex_pre_lock(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_pre_lock);
...
INTERFACE_ATTRIBUTE
void __trec_mutex_post_lock(void *m, unsigned flagz, int rec) {
  SCOPED_ANNOTATION(__trec_mutex_post_lock);
  ThreadIgnoreSyncEnd(thr, pc);
  ThreadIgnoreEnd(thr, pc);
...
INTERFACE_ATTRIBUTE
int __trec_mutex_pre_unlock(void *m, unsigned flagz) {
  SCOPED_ANNOTATION_RET(__trec_mutex_pre_unlock, 0);
  int ret = 0;
  if (flagz & MutexFlagReadLock) {
...
INTERFACE_ATTRIBUTE
void __trec_mutex_post_unlock(void *m, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_post_unlock);
  ThreadIgnoreSyncEnd(thr, pc);
  ThreadIgnoreEnd(thr, pc);
}

INTERFACE_ATTRIBUTE
void __trec_mutex_pre_signal(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_pre_signal);
  ThreadIgnoreBegin(thr, pc, /*save_stack=*/false);
  ThreadIgnoreSyncBegin(thr, pc, /*save_stack=*/false);
}

INTERFACE_ATTRIBUTE
void __trec_mutex_post_signal(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_post_signal);
  ThreadIgnoreSyncEnd(thr, pc);
  ThreadIgnoreEnd(thr, pc);
}

INTERFACE_ATTRIBUTE
void __trec_mutex_pre_divert(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_pre_divert);
  // Exit from ignore region started in __trec_mutex_pre_lock/unlock/signal.
  ThreadIgnoreSyncEnd(thr, pc);
  ThreadIgnoreEnd(thr, pc);
}

INTERFACE_ATTRIBUTE
void __trec_mutex_post_divert(void *addr, unsigned flagz) {
  SCOPED_ANNOTATION(__trec_mutex_post_divert);
  ThreadIgnoreBegin(thr, pc, /*save_stack=*/false);
  ThreadIgnoreSyncBegin(thr, pc, /*save_stack=*/false);
}
}  // extern "C"
