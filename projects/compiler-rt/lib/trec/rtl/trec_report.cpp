...
static bool FrameIsInternal(const SymbolizedStack *frame) {
  ...
  if (file != 0 &&
      (internal_strstr(file, "trec_interceptors_posix.cpp") ||
       internal_strstr(file, "sanitizer_common_interceptors.inc") ||
       internal_strstr(file, "trec_interface_")))
    return true;
  if (module != 0 && (internal_strstr(module, "libclang_rt.trec_")))
    return true;
  return false;
}

static SymbolizedStack *SkipTrecInternalFrames(SymbolizedStack *frames) {
...
void PrintReport(const ReportDesc *rep) {
  ...
  Printf("%s", d.Warning());
  Printf("WARNING: TraceRecorder: %s (pid=%d)\n", rep_typ_str,
         (int)internal_getpid());
  Printf("%s", d.Default());
  ...
  if (rep->typ == ReportTypeDeadlock) {
    ...
    for (uptr i = 0; i < rep->mutexes.Size(); i++) {
      ...
      if (flags()->second_deadlock_stack) {
        ...
        if (i == 0)
          Printf("    Hint: use TREC_OPTIONS=second_deadlock_stack=1 "
  ...
  if (ReportStack *stack = ChooseSummaryStack(rep)) {
    if (SymbolizedStack *frame = SkipTrecInternalFrames(stack->frames))
