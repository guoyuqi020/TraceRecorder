...
volatile int __trec_resumed = 0;

extern "C" void __trec_resume() { __trec_resumed = 1; }
...
bool OnFinalize(bool failed) { return failed; }
SANITIZER_WEAK_CXX_DEFAULT_IMPL
void OnInitialize() {}
#endif
...
Context::Context()
  ...
  if (trace_dir_env == nullptr) {
    internal_snprintf(trace_dir, 255, "/Data2/guoyq/program_results/mysql");
  } else
    internal_snprintf(trace_dir, 255, "%s", trace_dir_env);
  atomic_store(&global_id, 0, memory_order_relaxed);
  atomic_store(&forked_cnt, 0, memory_order_relaxed);
  char filepath[256];
  internal_snprintf(filepath, 255, "%s/trec_%d", trace_dir, pid);
...
void Context::CopyDir(const char *path, int Maintid) {
  ...
  internal_snprintf(parent_path, 255, "%s/trec_%d", trace_dir,
                    internal_getpid());
  ...
  internal_snprintf(dest_path, 511, "%s/header", path);
  if (mkdir(dest_path, 0700)) {
    Report("Create temp header directory failed\n");
    Die();
  }
  internal_snprintf(dest_path, 511, "%s/metadata", path);
  if (mkdir(dest_path, 0700)) {
    Report("Create temp metadata directory failed\n");
    Die();
  }
  internal_snprintf(dest_path, 511, "%s/trace", path);
  if (mkdir(dest_path, 0700)) {
    Report("Create temp trace directory failed\n");
    Die();
  }
  internal_snprintf(src_path, 511, "%s/header/%u.bin", parent_path, Maintid);
  internal_snprintf(dest_path, 511, "%s/header/%u.bin", path, Maintid);

  if (CopyFile(src_path, dest_path)) {
    Report("Parent copy bin header failed\n");
    Die();
  }
  internal_snprintf(src_path, 511, "%s/header/modules_%u.txt", parent_path,
                    Maintid);
  internal_snprintf(dest_path, 511, "%s/header/modules_%u.txt", path, Maintid);
  if (CopyFile(src_path, dest_path)) {
    Report("Parent copy module file failed\n");
    Die();
  }

  internal_snprintf(src_path, 511, "%s/metadata/%u.bin", parent_path, Maintid);
  internal_snprintf(dest_path, 511, "%s/metadata/%u.bin", path, Maintid);
  if (CopyFile(src_path, dest_path)) {
    Report("Parent copy metadata file failed\n");
    Die();
  }
  internal_snprintf(src_path, 511, "%s/trace/%u.bin", parent_path, Maintid);
  internal_snprintf(dest_path, 511, "%s/trace/%u.bin", path, Maintid);
  if (CopyFile(src_path, dest_path)) {
...
void Context::InheritDir(const char *path, uptr _pid) {
  char dirpath[256];
  internal_snprintf(dirpath, 255, "%s/trec_%llu", trace_dir, _pid);
...
void Context::open_directory(const char *path) {
  // open or create
  struct stat _st = {0};
  __sanitizer::internal_stat(path, &_st);
  char filepath[256];
  ...
  internal_snprintf(filepath, 255, "%s/%s", path, "trace");
  __sanitizer::internal_stat(filepath, &_st);
  if (!S_ISDIR(_st.st_mode)) {
    if (mkdir(filepath, S_IRWXU) != 0) {
      Report("Could not create directory at %s\n", filepath);
      Die();
    }
  }
  internal_snprintf(filepath, 255, "%s/%s", path, "metadata");
  __sanitizer::internal_stat(filepath, &_st);
  if (!S_ISDIR(_st.st_mode)) {
    if (mkdir(filepath, S_IRWXU) != 0) {
      Report("Could not create directory at %s\n", filepath);
  ...
  internal_snprintf(filepath, 255, "%s/%s", path, "header");
  __sanitizer::internal_stat(filepath, &_st);
  if (!S_ISDIR(_st.st_mode)) {
    if (mkdir(filepath, S_IRWXU) != 0) {
      Report("Could not create directory at %s\n", filepath);
...
static void *BackgroundThread(void *arg) {
  ...
  if (flags()->profile_memory && flags()->profile_memory[0]) {
    if (internal_strcmp(flags()->profile_memory, "stdout") == 0) {
      ...
      if (fd == kInvalidFd) {
        Printf("TraceRecorder: failed to open memory profile file '%s'\n",
               &filename[0]);
      } else {
        mprof_fd = fd;
      }
  ...
  for (int i = 0;
    ...
    if (flags()->flush_memory_ms > 0) {
      if (last_flush + flags()->flush_memory_ms * kMs2Ns < now) {
        VPrintf(1, "TraceRecorder: periodic memory flush\n");
        FlushShadowMemory();
        last_flush = NanoTime();
      }
    }
    ...
    if (flags()->memory_limit_mb > 0) {
      ...
      VPrintf(1,
              "TraceRecorder: memory flush check"
              " RSS=%llu LAST=%llu LIMIT=%llu\n",
              (u64)rss >> 20, (u64)last_rss >> 20, (u64)limit >> 20);
      if (2 * rss > limit + last_rss) {
        VPrintf(1, "TraceRecorder: flushing memory due to RSS\n");
        FlushShadowMemory();
        rss = GetRSS();
        VPrintf(1, "TraceRecorder: memory flushed RSS=%llu\n", (u64)rss >> 20);
      }
      last_rss = rss;
    }
...
void MapThreadTrace(uptr addr, uptr size, const char *name) {
  ...
  if (!MmapFixedSuperNoReserve(addr, size, name)) {
    Printf("FATAL: TraceRecorder can not mmap thread trace (%p/%p)\n", addr,
           size);
    Die();
  }
}
...
void Initialize(ThreadState *thr) {
  ...
  SetCheckFailedCallback(TrecCheckFailed);
  ...
  if (flags()->stop_on_start) {
    Printf(
        "TraceRecorder is suspended at startup (pid %d)."
        " Call __trec_resume().\n",
        (int)internal_getpid());
    while (__trec_resumed == 0) {
    }
  }
...
void MaybeSpawnBackgroundThread() {
  // On MIPS, TRec initialization is run before
...
int Finalize(ThreadState *thr) {
  ...
  if (ctx->nreported) {
    failed = true;
#if !SANITIZER_GO
    Printf("TraceRecorder: reported %d warnings\n", ctx->nreported);
#else
  ...
  if (ctx->nmissed_expected) {
    failed = true;
    Printf("TraceRecorder: missed %d expected races\n", ctx->nmissed_expected);
  }
...
void GrowShadowStack(ThreadState *thr) {
  ...
  uptr *newstack =
      (uptr *)internal_alloc(MBlockShadowStack, newsz * sizeof(uptr));
...
Trace *ThreadTrace(int tid) { return (Trace *)GetThreadTraceHeader(tid); }
...
uptr TraceParts() { return TraceSize() / kTracePartSize; }

#if !SANITIZER_GO
extern "C" void __trec_trace_switch() { TraceSwitch(cur_thread()); }

extern "C" void __trec_report_race() { ReportRace(cur_thread()); }
#endif
...
#include "trec_update_shadow_word_inl.h"
  }
#else
  int idx = 0;
#include "trec_update_shadow_word_inl.h"
...
void UnalignedMemoryAccess(ThreadState *thr, uptr pc, uptr addr, int size,
  ...
  while (size) {
    ...
    MemoryAccess(thr, pc, addr, kAccessSizeLog, kAccessIsWrite, kIsAtomic,
                 isPtr, real_val, SAI_addr, SAI_val);
    addr += size1;
    size -= size1;
  }
}
...
#define SHUF(v0, v1, i0, i1, i2, i3)                                          \
  _mm_castps_si128(_mm_shuffle_ps(_mm_castsi128_ps(v0), _mm_castsi128_ps(v1), \
...
ALWAYS_INLINE USED void FuncParam(ThreadState *thr, char param_idx,
                                  uptr src_addr, char src_idx) {
  if (UNLIKELY(!ctx->flags.record_func_param))
    return;
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::FuncParam,
        atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
        thr->tctx->metadata_offset);
    ...
    thr->tctx->header.StateInc(__trec_header::RecordType::FuncParam);
  }
}
...
ALWAYS_INLINE USED void FuncExitParam(ThreadState *thr, uptr src_addr,
  ...
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::FuncExitParam,
        atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
        thr->tctx->metadata_offset);

    thr->tctx->put_trace(&e, sizeof(__trec_trace::Event));
    __trec_metadata::FuncExitParamMeta meta(src_idx, src_addr);
    thr->tctx->put_metadata(&meta, sizeof(meta));
  }
}

ALWAYS_INLINE USED void FuncEnterOrder(ThreadState *thr, char idx) {
  if (UNLIKELY(!ctx->flags.record_func_param))
    return;
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::FuncEnterOrder,
        atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
        thr->tctx->metadata_offset);

    thr->tctx->put_trace(&e, sizeof(__trec_trace::Event));
    __trec_metadata::FuncEnterOrderMeta meta(idx);
    thr->tctx->put_metadata(&meta, sizeof(meta));
  }
}
...
ALWAYS_INLINE USED void MemoryAccess(
    ...
    __trec_metadata::SourceAddressInfo SAI_addr,
    __trec_metadata::SourceAddressInfo SAI_val) {
  if (!(ctx->flags.no_stack && IsStackMem(addr))) {
    if (LIKELY(ctx->flags.output_trace)) {
      if (kAccessIsWrite) {
        __trec_trace::Event e(
            isPtr ? __trec_trace::EventType::PtrWrite
                  : __trec_trace::EventType::PlainWrite,
            atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
            thr->tctx->metadata_offset);

        thr->tctx->put_trace(&e, sizeof(__trec_trace::Event));
        __trec_metadata::WriteMeta meta(addr, val, pc, SAI_addr.idx,
                                        SAI_addr.addr, SAI_val.idx,
                                        SAI_val.addr);
        thr->tctx->put_metadata(&meta, sizeof(meta));
        thr->tctx->header.StateInc(isPtr
                                       ? __trec_header::RecordType::PtrWrite
                                       : __trec_header::RecordType::PlainWrite);
      } else {
        __trec_trace::Event e(
        ...
        thr->tctx->put_trace(&e, sizeof(__trec_trace::Event));
        __trec_metadata::ReadMeta meta(addr, val, pc, SAI_addr.idx,
                                       SAI_addr.addr);
        thr->tctx->put_metadata(&meta, sizeof(meta));
        thr->tctx->header.StateInc(isPtr
...
// Called by MemoryAccessRange in trec_rtl_thread.cpp
...
ALWAYS_INLINE USED void RecordFuncEntry(ThreadState *thr, uptr pc) {
  if (UNLIKELY(!ctx->flags.record_func_enter_exit))
    return;
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::FuncEnter,
        atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
        thr->tctx->metadata_offset);
...
ALWAYS_INLINE USED void RecordFuncExit(ThreadState *thr) {
  ...
  if (LIKELY(ctx->flags.output_trace)) {
    __trec_trace::Event e(
        __trec_trace::EventType::FuncExit,
        atomic_fetch_add(&ctx->global_id, 1, memory_order_relaxed),
        thr->tctx->metadata_offset);
...
__trec_testonly_shadow_stack_current_size() {
