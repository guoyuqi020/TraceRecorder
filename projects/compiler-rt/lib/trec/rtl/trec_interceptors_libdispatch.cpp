//===-- trec_interceptors_libdispatch.cpp
//---------------------------------===//
//
...
static block_context_t *AllocContext(ThreadState *thr, uptr pc,
                                     ...
                                     dispatch_function_t orig_work) {
  block_context_t *new_context = (block_context_t *)user_alloc_internal(
      thr, pc, sizeof(block_context_t), kDefaultAlignment, false);
...
static void dispatch_sync_pre_execute(ThreadState *thr, uptr pc,
  ...
  do {
    ...
    if (serial_task && concurrent_sync)
      Acquire(thr, pc, concurrent_sync);

    if (q)
      q = GetTargetQueueFromQueue(q);
  } while (q);
}
...
static void dispatch_sync_post_execute(ThreadState *thr, uptr pc,
                                       block_context_t *context) {
  uptr submit_sync = (uptr)context;
  if (context->submitted_synchronously)
    Release(thr, pc, submit_sync);

  dispatch_queue_t q = context->queue;
  do {
    GET_QUEUE_SYNC_VARS(context, q);
    if (serial_task && serial_sync)
      Release(thr, pc, serial_sync);
    if (!serial_task && concurrent_sync)
      Release(thr, pc, concurrent_sync);
...
static void dispatch_callback_wrap(void *param) {
  ...
  if (context->free_context_in_callback)
...
#define DISPATCH_INTERCEPT_ASYNC_B(name, barrier)                            \
  TREC_INTERCEPTOR(void, name, dispatch_queue_t q, dispatch_block_t block) { \
    SCOPED_TREC_INTERCEPTOR(name, q, block);                                 \
    SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_START();                           \
...
#define DISPATCH_INTERCEPT_SYNC_B(name, barrier)                               \
  TREC_INTERCEPTOR(void, name, dispatch_queue_t q,                             \
                   DISPATCH_NOESCAPE dispatch_block_t block) {                 \
    SCOPED_TREC_INTERCEPTOR(name, q, block);                                   \
    block_context_t new_context = {q,       block, &invoke_block, false, true, \
                                   barrier, 0};                                \
    Release(thr, pc, (uptr)&new_context);                                      \
    SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_START();                             \
    REAL(name##_f)(q, &new_context, dispatch_callback_wrap);                   \
...
#define DISPATCH_INTERCEPT_ASYNC_F(name, barrier)                           \
  TREC_INTERCEPTOR(void, name, dispatch_queue_t q, void *context,           \
                   dispatch_function_t work) {                              \
    SCOPED_TREC_INTERCEPTOR(name, q, context, work);                        \
    block_context_t *new_context = AllocContext(thr, pc, q, context, work); \
    new_context->is_barrier_block = barrier;                                \
    Release(thr, pc, (uptr)new_context);                                    \
    SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_START();                          \
...
#define DISPATCH_INTERCEPT_SYNC_F(name, barrier)                               \
  TREC_INTERCEPTOR(void, name, dispatch_queue_t q, void *context,              \
                   dispatch_function_t work) {                                 \
    SCOPED_TREC_INTERCEPTOR(name, q, context, work);                           \
    block_context_t new_context = {q, context, work, false, true, barrier, 0}; \
    Release(thr, pc, (uptr)&new_context);                                      \
    SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_START();                             \
    REAL(name)(q, &new_context, dispatch_callback_wrap);                       \
    SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_END();                               \
...
// generate a weak reference so the TRec dylib continues to work on older
...
SANITIZER_WEAK_IMPORT void dispatch_async_and_wait_f(dispatch_queue_t queue,
                                                     void *context,
                                                     dispatch_function_t work);
...
TREC_INTERCEPTOR(void, dispatch_after, dispatch_time_t when,
                 dispatch_queue_t queue, dispatch_block_t block) {
  SCOPED_TREC_INTERCEPTOR(dispatch_after, when, queue, block);
  SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_START();
  dispatch_block_t heap_block = Block_copy(block);
  SCOPED_TREC_INTERCEPTOR_USER_CALLBACK_END();
  block_context_t *new_context =
...
TREC_INTERCEPTOR(void, dispatch_after_f, dispatch_time_t when,
                 dispatch_queue_t queue, void *context,
                 dispatch_function_t work) {
  SCOPED_TREC_INTERCEPTOR(dispatch_after_f, when, queue, context, work);
  WRAP(dispatch_after)
  (when, queue, ^(void) {
    work(context);
  });
...
// and acquire semantics. Since TRec does not see its own atomic stores,
// the race on predicate won't be reported - the only accesses to it that
// TRec sees are the loads on the fast path. Loads don't race. Secondly,
// dispatch_once is both a macro and a real function, we want to intercept the
// function, so we need to undefine the macro.
#undef dispatch_once
TREC_INTERCEPTOR(void, dispatch_once, dispatch_once_t *predicate,
                 DISPATCH_NOESCAPE dispatch_block_t block) {
...
TREC_INTERCEPTOR(void, dispatch_once_f, dispatch_once_t *predicate,
...
TREC_INTERCEPTOR(long_t, dispatch_semaphore_signal,
                 dispatch_semaphore_t dsema) {
  SCOPED_TREC_INTERCEPTOR(dispatch_semaphore_signal, dsema);
  Release(thr, pc, (uptr)dsema);
  return REAL(dispatch_semaphore_signal)(dsema);
}

TREC_INTERCEPTOR(long_t, dispatch_semaphore_wait, dispatch_semaphore_t dsema,
                 dispatch_time_t timeout) {
  SCOPED_TREC_INTERCEPTOR(dispatch_semaphore_wait, dsema, timeout);
  long_t result = REAL(dispatch_semaphore_wait)(dsema, timeout);
  if (result == 0)
    Acquire(thr, pc, (uptr)dsema);
  return result;
}

TREC_INTERCEPTOR(long_t, dispatch_group_wait, dispatch_group_t group,
                 dispatch_time_t timeout) {
  SCOPED_TREC_INTERCEPTOR(dispatch_group_wait, group, timeout);
...
TREC_INTERCEPTOR(void, dispatch_group_leave, dispatch_group_t group) {
  SCOPED_TREC_INTERCEPTOR(dispatch_group_leave, group);
...
TREC_INTERCEPTOR(void, dispatch_group_async, dispatch_group_t group,
                 dispatch_queue_t queue, dispatch_block_t block) {
  SCOPED_TREC_INTERCEPTOR(dispatch_group_async, group, queue, block);
...
TREC_INTERCEPTOR(void, dispatch_group_async_f, dispatch_group_t group,
                 dispatch_queue_t queue, void *context,
                 dispatch_function_t work) {
  SCOPED_TREC_INTERCEPTOR(dispatch_group_async_f, group, queue, context, work);
  dispatch_retain(group);
  dispatch_group_enter(group);
  WRAP(dispatch_async)
  (queue, ^(void) {
...
TREC_INTERCEPTOR(void, dispatch_group_notify, dispatch_group_t group,
                 dispatch_queue_t q, dispatch_block_t block) {
  SCOPED_TREC_INTERCEPTOR(dispatch_group_notify, group, q, block);
...
TREC_INTERCEPTOR(void, dispatch_group_notify_f, dispatch_group_t group,
                 dispatch_queue_t q, void *context, dispatch_function_t work) {
  WRAP(dispatch_group_notify)
  (group, q, ^(void) {
    work(context);
  });
}

TREC_INTERCEPTOR(void, dispatch_source_set_event_handler,
                 dispatch_source_t source, dispatch_block_t handler) {
  SCOPED_TREC_INTERCEPTOR(dispatch_source_set_event_handler, source, handler);
  if (handler == nullptr)
...
TREC_INTERCEPTOR(void, dispatch_source_set_event_handler_f,
                 dispatch_source_t source, dispatch_function_t handler) {
  SCOPED_TREC_INTERCEPTOR(dispatch_source_set_event_handler_f, source, handler);
  if (handler == nullptr)
    return REAL(dispatch_source_set_event_handler)(source, nullptr);
...
TREC_INTERCEPTOR(void, dispatch_source_set_cancel_handler,
                 dispatch_source_t source, dispatch_block_t handler) {
  SCOPED_TREC_INTERCEPTOR(dispatch_source_set_cancel_handler, source, handler);
  if (handler == nullptr)
...
TREC_INTERCEPTOR(void, dispatch_source_set_cancel_handler_f,
                 dispatch_source_t source, dispatch_function_t handler) {
  SCOPED_TREC_INTERCEPTOR(dispatch_source_set_cancel_handler_f, source,
                          handler);
  if (handler == nullptr)
    return REAL(dispatch_source_set_cancel_handler)(source, nullptr);
  dispatch_block_t block = ^(void) {
...
TREC_INTERCEPTOR(void, dispatch_source_set_registration_handler,
                 dispatch_source_t source, dispatch_block_t handler) {
  SCOPED_TREC_INTERCEPTOR(dispatch_source_set_registration_handler, source,
                          handler);
  if (handler == nullptr)
...
TREC_INTERCEPTOR(void, dispatch_source_set_registration_handler_f,
                 dispatch_source_t source, dispatch_function_t handler) {
  SCOPED_TREC_INTERCEPTOR(dispatch_source_set_registration_handler_f, source,
                          handler);
  if (handler == nullptr)
    return REAL(dispatch_source_set_registration_handler)(source, nullptr);
  dispatch_block_t block = ^(void) {
...
TREC_INTERCEPTOR(void, dispatch_apply, size_t iterations,
                 dispatch_queue_t queue,
                 DISPATCH_NOESCAPE void (^block)(size_t)) {
  SCOPED_TREC_INTERCEPTOR(dispatch_apply, iterations, queue, block);
...
TREC_INTERCEPTOR(void, dispatch_apply_f, size_t iterations,
                 dispatch_queue_t queue, void *context,
                 void (*work)(void *, size_t)) {
  SCOPED_TREC_INTERCEPTOR(dispatch_apply_f, iterations, queue, context, work);
...
TREC_INTERCEPTOR(dispatch_data_t, dispatch_data_create, const void *buffer,
                 size_t size, dispatch_queue_t q, dispatch_block_t destructor) {
  SCOPED_TREC_INTERCEPTOR(dispatch_data_create, buffer, size, q, destructor);
  ...
  if (destructor == DISPATCH_DATA_DESTRUCTOR_FREE)
    destructor = ^(void) {
      WRAP(free)((void *)(uintptr_t)buffer);
    };
  else if (destructor == DISPATCH_DATA_DESTRUCTOR_MUNMAP)
    destructor = ^(void) {
      WRAP(munmap)((void *)(uintptr_t)buffer, size);
    };
...
TREC_INTERCEPTOR(void, dispatch_read, dispatch_fd_t fd, size_t length,
                 dispatch_queue_t q, fd_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_read, fd, length, q, h);
  __block block_context_t new_context = {
...
TREC_INTERCEPTOR(void, dispatch_write, dispatch_fd_t fd, dispatch_data_t data,
                 dispatch_queue_t q, fd_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_write, fd, data, q, h);
  __block block_context_t new_context = {
      q, nullptr, &invoke_block, false, false, false, 0};
  fd_handler_t new_h = Block_copy(^(dispatch_data_t data, int error) {
    new_context.orig_context = ^(void) {
...
TREC_INTERCEPTOR(void, dispatch_io_read, dispatch_io_t channel, off_t offset,
                 size_t length, dispatch_queue_t q, dispatch_io_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_read, channel, offset, length, q, h);
  __block block_context_t new_context = {
      q, nullptr, &invoke_block, false, false, false, 0};
...
TREC_INTERCEPTOR(void, dispatch_io_write, dispatch_io_t channel, off_t offset,
                 dispatch_data_t data, dispatch_queue_t q,
                 dispatch_io_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_write, channel, offset, data, q, h);
  __block block_context_t new_context = {
      q, nullptr, &invoke_block, false, false, false, 0};
  dispatch_io_handler_t new_h =
      Block_copy(^(bool done, dispatch_data_t data, int error) {
...
TREC_INTERCEPTOR(void, dispatch_io_barrier, dispatch_io_t channel,
                 dispatch_block_t barrier) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_barrier, channel, barrier);
  __block block_context_t new_context = {
...
TREC_INTERCEPTOR(dispatch_io_t, dispatch_io_create, dispatch_io_type_t type,
                 dispatch_fd_t fd, dispatch_queue_t q, cleanup_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_create, type, fd, q, h);
...
TREC_INTERCEPTOR(dispatch_io_t, dispatch_io_create_with_path,
                 dispatch_io_type_t type, const char *path, int oflag,
                 mode_t mode, dispatch_queue_t q, cleanup_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_create_with_path, type, path, oflag, mode,
                          q, h);
  __block dispatch_io_t new_channel = nullptr;
  __block block_context_t new_context = {
      q, nullptr, &invoke_block, false, false, false, 0};
...
TREC_INTERCEPTOR(dispatch_io_t, dispatch_io_create_with_io,
                 dispatch_io_type_t type, dispatch_io_t io, dispatch_queue_t q,
                 cleanup_handler_t h) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_create_with_io, type, io, q, h);
  __block dispatch_io_t new_channel = nullptr;
  __block block_context_t new_context = {
      q, nullptr, &invoke_block, false, false, false, 0};
  cleanup_handler_t new_h = Block_copy(^(int error) {
...
TREC_INTERCEPTOR(void, dispatch_io_close, dispatch_io_t channel,
                 dispatch_io_close_flags_t flags) {
  SCOPED_TREC_INTERCEPTOR(dispatch_io_close, channel, flags);
...
TREC_INTERCEPTOR(void, dispatch_resume, dispatch_object_t o) {
  SCOPED_TREC_INTERCEPTOR(dispatch_resume, o);
